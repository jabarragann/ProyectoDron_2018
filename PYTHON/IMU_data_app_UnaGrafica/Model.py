...
import numpy as np
from numpy import sin, cos, tan, arctan, arctan2,sqrt,pi
...
class Model():
    ...
    def calculateAttitudeAccel(self,accel_x,accel_y,accel_z,index):
        ...
        self.roll_accel[index]  = arctan2(accel_y, accel_z);
        self.pitch_accel[index] = arctan2(-accel_x, sqrt(accel_y * accel_y + accel_z * accel_z));
    ...
    def calculateComplementaryFilter2(self,index,accel_x,accel_y,accel_z,gyr_x,gyr_y,gyr_z,l=0.98):
        
        self.pitch_accel[index] = arctan(-1*(accel_x)/sqrt( accel_y**2 + accel_z**2) ) *180/pi;
        self.roll_accel[index]  = arctan((accel_y)/sqrt(accel_x**2 + accel_z**2))*180/pi;
           
        self.roll[index]  = l *(self.roll[index-1]+gyr_x*0.010) + (1-l)*self.roll_accel[index];
        self.pitch[index] = l *(self.pitch[index-1]+gyr_y*0.010)+ (1-l)*self.pitch_accel[index];
